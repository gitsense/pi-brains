...
const UPSERT_SQL = `
  INSERT INTO heartbeats (session_id, pid, cwd, started_at, status, last_heartbeat_at)
  VALUES (?, ?, ?, ?, ?, ?)
...
const SELECT_COLUMNS = "session_id, pid, cwd, started_at, status, last_heartbeat_at";
...
export function startSessionHeartbeat(
  ...
  const write = (status: "alive" | "stopped"): void => {
    ...
    try {
      ...
      const now = Date.now();
      store.upsert.run(sessionId, process.pid, ctx.cwd, startedAt, status, now);
