...
const PI_AGENT_CONTEXT: AgentContext = {
  id: "pi",
  displayName: "Pi",
  adapter: "pi-brains",
  session: {
...
export class RuleEngine {
  ...
  private buildExecutionContext(
    ...
    return {
      ...
      event: {
        name: lifecycleEvent,
        // gsc rules execute still uses runtime when formatting its human-readable block reason.
        // gsc strips this legacy field before passing context to executable triggers.
        runtime: "pi",
        runtimeEvent: "tool_call",
      },
      agent: PI_AGENT_CONTEXT,
      capabilities: {
  ...
  private buildMatchedRulePacket(
    ...
    for (const packet of matchedPackets) {
      if (packet.type !== "executable") continue;
      ...
      lines.push(`${ruleIndex}. ${title} [executable]`);
      lines.push(`   Rule: ${rule.rule.id}`);
      lines.push(`   Match: ${matchKind}: ${matchValue}`);
  ...
  private buildTriggerContext(
    ...
    const baseContext: V1TriggerContext = {
      ...
      rule: {
        ...
        type: isTriggerRule(matchedRule) ? "executable" : "instruction",
...
function isTriggerRule(matchedRule: MatchedGscRule): boolean {
  return (matchedRule.rule.type === "tool-trigger" || matchedRule.rule.type === "executable") && !!matchedRule.rule.trigger;
}
